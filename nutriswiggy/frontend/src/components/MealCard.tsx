...
export const MealCard: React.FC<{ 
  ...
  // Set health score color schemes adapted for dark mode
  const getScoreColor = (score: number) => {
    if (score >= 80) return "text-emerald-400 border-emerald-500/30 bg-emerald-500/10 shadow-sm shadow-emerald-500/10";
    if (score >= 60) return "text-amber-400 border-amber-500/30 bg-amber-500/10 shadow-sm shadow-amber-500/10";
    return "text-rose-400 border-rose-500/30 bg-rose-500/10 shadow-sm shadow-rose-500/10";
  };

  return (
    <div className="bg-slate-900/90 border border-slate-800/80 backdrop-blur-md rounded-2xl p-5 flex flex-col justify-between h-full relative overflow-hidden group shadow-xl">
      
      {/* Decorative gradient glowing orb inside card */}
      <div className="absolute -right-12 -top-12 w-24 h-24 bg-[#FC8019]/10 rounded-full blur-2xl group-hover:bg-[#FC8019]/20 transition-all duration-500" />
      ...
      <div>
        ...
        <div className="flex justify-between items-start mb-3">
          <div className="flex items-center gap-2">
            ...
            <div className={`w-4 h-4 border-2 rounded flex items-center justify-center p-0.5 ${veg ? "border-emerald-500" : "border-rose-500"}`}>
              <div className={`w-1.5 h-1.5 rounded-full ${veg ? "bg-emerald-500" : "bg-rose-500"}`} />
            </div>
            <span className="text-xs font-bold text-slate-400 tracking-wider uppercase truncate max-w-[150px]">{restaurant}</span>
          </div>
          <span className="text-sm font-black text-white">₹{price}</span>
        </div>

        {/* Meal Name */}
        <h3 className="text-lg font-bold text-white mb-2 leading-tight group-hover:text-[#FC8019] transition-colors duration-300">
          {item}
        </h3>
          ...
          {badges.map((badge, idx) => (
            <span
              ...
              className={`text-[10px] font-bold px-2 py-0.5 rounded-full uppercase tracking-wider border ${
                badge === "Top Pick"
                  ? "bg-gradient-to-r from-[#FC8019] to-amber-500 border-[#FC8019]/30 text-white shadow-sm shadow-[#FC8019]/20"
        ...
        <div className="bg-slate-950/60 border border-slate-800/80 rounded-xl p-3 mb-4 shadow-sm">
          <div className="flex justify-between items-center mb-2">
            ...
            <span className="text-[10px] text-slate-400 font-medium">Estimated Macros</span>
          </div>
          ...
          <div className="grid grid-cols-2 gap-x-4 gap-y-2 mt-2 text-[11px]">
            ...
            <div>
              <div className="flex justify-between text-slate-400 mb-0.5">
                <span>Protein</span>
                <span className="font-bold text-emerald-400">{macros.protein}g</span>
              </div>
      ...
      <div className="mt-auto pt-3 border-t border-slate-800/80">
        ...
        <div className="flex flex-col gap-1 mb-3">
          ...
          {penalties_applied.length > 0 && (
            <div className="flex items-center gap-1 text-[10px] text-rose-400 font-semibold bg-rose-500/10 px-2 py-0.5 rounded border border-rose-500/20">
        ...
        <div className="bg-slate-950/60 border border-slate-800/80 border-l-2 border-l-[#FC8019] rounded-r-lg p-2.5 mb-3">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className={`px-3 py-1.5 rounded-xl text-xs font-black tracking-wide transition-all duration-200 active:scale-95 flex items-center gap-1 border ${
                isInCart
                  ? "bg-gradient-to-r from-[#FC8019] to-amber-500 text-white border-[#FC8019]/30 hover:opacity-90 shadow-md shadow-[#FC8019]/20"
                  : "bg-slate-800 hover:bg-slate-700 text-[#FC8019] hover:text-amber-500 border border-slate-700 hover:border-[#FC8019]/50"
              }`}
            >
              {isInCart ? (
                <>
